...
var pallete = []color.Color{color.Black, color.RGBA{0, 255, 0, 1}, color.RGBA{255, 0, 0, 1}, color.RGBA{0, 0, 255, 1}}
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(random(1, 4)))
		}
		phase += 0.1
